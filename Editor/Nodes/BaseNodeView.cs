    ...
    public abstract class BaseNodeView<T> : Node, IGroupable, INodeView where T: INode
    {
        ...
        private void AddControl(CustomPort port, PropertyInfo property)
        {
            ...
            else if (typeof(UnityEngine.Object).IsAssignableFrom(property.PropertyType))
            {
                ...
                field.objectType = property.PropertyType;

                // Use direct casting to set the value without notifying.
                var currentValue = property.GetValue(Data) as UnityEngine.Object;
                field.SetValueWithoutNotify(currentValue);

                // Register the callback. The callback will receive a UnityEngine.Object,
                // but SetValue can handle the specific type via reflection.
                field.RegisterCallback<ChangeEvent<UnityEngine.Object>>(e =>
                {
                    // Since Unity uses UnityEngine.Object, casting to the specific type
                    // is not necessary for setting the value.
                    property.SetValue(Data, e.newValue);
                });
            ...
            // else
            // {
            //     var label = new Label($"Unsupported type: {property.PropertyType.Name}. {property is UnityEngine.Object}");
            //     
            //     var wrapper = new VisualElement();
            //     wrapper.AddToClassList("p-5");
            //     wrapper.Add(label);
