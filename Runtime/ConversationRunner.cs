...
namespace Conversa.Runtime
{
	public class ConversationRunner
	{
		...
		public ConversationRunner(Conversation conversation)
		{
			...
			OnConversationEvent.AddListener(HandleConversationEvents);

		}

		public void ResetProperties() => clone.GetValueProperties().ToList().ForEach(x => x.Reset());

		// Update isRunning if the conversation ends
		private void HandleConversationEvents(IConversationEvent e)
		{
			if (e is EndEvent) isRunning = false;
		}
		...
		public void BeginByGuid(string nodeGuid)
		{
			...
			if (startPoint is IEventNode eventNode)
				BeginOnNode(eventNode);
			else
				Begin();
		}

		private void BeginOnNode(IEventNode node)
		{
			isRunning = true;
		...
		public void Begin(string bookmark) => BeginOnNode(clone.GetBookmarkNode(bookmark));
